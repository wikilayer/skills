...
1. **Em-dash.** The em-dash (`—`) is a rare mark, earned, not default punctuation. Flag every occurrence and judge whether it is the seldom case where nothing else carries the meaning; almost always a comma, colon, period, or parentheses is the fix. Do not wave `—` through as "one per page is fine".
   ...
   First check whether the facet's language requires the mark by grammar rather than choosing it for effect. Where it does, the dash is spelling and stays, and this check does not run on it: Russian puts one between subject and predicate when the copula is absent, and flagging those is a false finding on every page in the wiki.
