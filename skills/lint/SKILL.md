...
1. Resolve the target from `$ARGUMENTS` (numeric id or URL). If empty, ask.
   ...
   **Scope: whole wiki or one page.** The target may be a whole wiki or a single page (its subtree). Read its `kind` with `get_outline` or `get_node`. A `wiki` target runs the full procedure; a `page` target lints only that page and its blocks (the per-block checks) and skips the wiki-level graph checks (rule 10), which need the whole tree. Use a page target for a focused pass after editing one page; a wiki target for a full audit.
...
4. **(Wiki target only.)** Caller runs the wiki-level graph checks (rule 10 below) using `search_nodes(<wiki-id>, body_contains=<pattern>, language=<facet>)` queries; the hits alone tell the story.
