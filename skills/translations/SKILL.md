...
1. Resolve the target and target language from the user's request (for example "wiki 1 to en", or a URL plus a BCP 47 code). The wiki's `primary_language` is the source. If either is unclear, ask.
...
2. Read and accept every rules page the Wikilayer server requires before protected reads or writes, then carry the returned agreement tokens on every call they bind. Translations must follow the [writing style](https://wikilayer.org/smee-again/wikilayer-howto/2992-writing-style) and avoid the [anti-patterns](https://wikilayer.org/smee-again/wikilayer-howto/3038-anti-patterns), same as any other write.
