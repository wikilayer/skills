...
4. Spawn one general-purpose subagent per source page in scope (the whole wiki, or just the target page's subtree; skip pages whose whole subtree is already linked, and pages tagged `i18n-exempt`). Each subagent:
   - Reads the source page's **exact** content with `get_page_markdown(<page-id>, limit=100, offset=0)`, paging with `offset` while `has_more` is true. Read the verbatim source, never WebFetch the `.md` URL, which a model can reword. The `<!-- block:N -->` comment above each node is the id to link a translation to; the closing `## Links here` section is generated navigation and is never translated.
