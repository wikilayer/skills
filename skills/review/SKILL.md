...
3. Spawn one general-purpose subagent per page. Each subagent reads the page's **exact** content with `get_page_markdown(<page-id>)`, in reading order already. It runs the per-page categories below and returns a verdict in two parts.
